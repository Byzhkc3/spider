    ...
    def getCode(self):
        ...
        def acquireCode():
            ...
            try:
                ...
                return dict(code=2000, cookies_temp = self.browser.get_cookies())
            finally:
                self.browser.close()
        # 逻辑
        if openBrowser():
    ...
    def loginSys(self):
        ...
        def switch_iframe():
            try:
                ...
                print '无法跳转到登陆的iframe',frame_ex
            try:
                self.browser.find_element_by_id('btn_get_dpw')
            except ElementNotVisibleException as ex:
                if self.refresh_num > 0:
                    self.refresh_num -= 1
                    self.browser.refresh()
                    return switch_iframe()  # 调用自身
                else:
                    raise Exception(u'对不起,获取手机动态码失败')
            else:
                return login()
...
def getNoteCode():
    ...
    :return:
    """
    _phone_attr = {'phone':'15802027662', 'province':'广东', 'city':'广州', 'company':2, 'password':'20168888'}
    spider = ChinaMobile_GD(_phone_attr)
    result = spider.getCode()
    if result['code'] == 2000:
        return result   # dict(code=2000, cookies_temp = self.browser.get_cookies())
    else:
