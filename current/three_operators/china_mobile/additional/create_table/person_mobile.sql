...
# 中国移动-用户信息表
drop table if exists `t_china_mobile_user`;
create table `t_china_mobile_user`(
    ...
    `cert_id` char(18) not null comment '证件号码',

    `phone` varchar(16)  not null comment '手机号',
    `home`  varchar(128) default null comment '归属地',
    `brand` varchar(32)  default null comment '用户套餐',
...
)engine=innodb default charset=utf8 auto_increment=1000 comment='中国移动-用户信息表';


# 中国移动-用户通话记录表
drop table if exists `t_china_mobile_call`;
create table `t_china_mobile_call`(

	`id` int unsigned primary key auto_increment,
    `alter_time` timestamp not null default current_timestamp comment '创建时间',
	
    `cert_id` char(18) comment '身份证号/指回t_china_unicom_uesr.cert_id',
...
)engine=innodb default charset=utf8 auto_increment=1000 comment='中国移动-通话记录表';
